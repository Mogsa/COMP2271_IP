...
class BM3D:
    ...
    def _find_similar_blocks(self, image, ref_y, ref_x, window_size=33):
        ...
        ref_block = image[ref_y:ref_y+self.block_size, ref_x:ref_x+self.block_size]
    ...
    def _process_group(self, image, coords):
        ...
        # Extract blocks
        stack = np.zeros((len(coords), self.block_size, self.block_size), dtype=np.float32)
        ...
        for i in range(len(coords)):
            dct_stack[i] = fftpack.dct(fftpack.dct(stack[i].T, norm='ortho').T, norm='ortho')
        ...
        for i in range(len(coords)):
            result[i] = fftpack.idct(fftpack.idct(idct_stack[i].T, norm='ortho').T, norm='ortho')
        ...
        for y in range(0, padded_h-self.block_size+1, 6):  # Step size 6 for speed
            for x in range(0, padded_w-self.block_size+1, 6):
                ...
                for i, (by, bx) in enumerate(coords):
                    padded_result[by:by+self.block_size, bx:bx+self.block_size] += denoised_blocks[i]
                    padded_weight[by:by+self.block_size, bx:bx+self.block_size] += 1
