...
class EnhancedColorCorrector:
    ...
    def analyze_image(self, image):
        ...
        if np.any(non_black):
            ...
            metrics["luminance"] = (
                0.299 * metrics["mean_r"]
                + 0.587 * metrics["mean_g"]
                + 0.114 * metrics["mean_b"]
            )
            ...
            # Check contrast
            metrics["contrast"] = max(
                metrics["std_r"], metrics["std_g"], metrics["std_b"]
            )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Enhanced automatic color correction without reference learning."
    )
    parser.add_argument(
