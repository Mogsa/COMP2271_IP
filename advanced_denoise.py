...
class AdvancedDenoiser:
    ...
    def bm3d_denoising(
        self, image, strength=0.7, apply_median_first=True, median_kernel_size=3
    ):
        """
        ...
        if apply_median_first:
            if self.debug:
                print(
                    f"Applying median filter with kernel size {median_kernel_size}x{median_kernel_size}"
                )
        ...
        denoiser = BM3D(sigma=sigma, block_size=8, max_blocks=16, hard_threshold=2.7)
    ...
    def nlm_color_channels(self, image, preserve_detail_level=0.7):
        ...
        cr_h = int(
            25 * (1.0 - preserve_detail_level * 0.5)
        )  # Much more aggressive for chrominance
        ...
        final_result[non_black & edge_mask] = result[
            non_black & edge_mask
        ]  # Keep NLM only near edges
        final_result[non_black & ~edge_mask] = bilateral_result[
            non_black & ~edge_mask
        ]  # Bilateral elsewhere
    ...
    def multi_stage_denoising(self, image, preserve_detail_level=0.7):
        ...
        if self.debug:
            print(
                f"Denoising parameters - Luminance: {h_luminance}, Chrominance: {h_chrominance}"
            )
    ...
    def adaptive_edge_aware_denoising(self, image, preserve_detail_level=0.85):
        ...
        _, sobel_thresh = cv2.threshold(
            sobel, 120, 255, cv2.THRESH_BINARY
        ...
        edges1 = cv2.Canny(
            sobel, 150, 300
        )  # Significantly increased threshold for very strong edges only
        edges2 = cv2.Canny(
            enhanced_gray, 100, 200
        )  # Increased threshold for medium edges

        # Combine edge detection results - prioritize Canny over Sobel since Sobel is too sensitive
        edges = cv2.bitwise_or(
            edges1, edges2
        )  # Removed sobel_cleaned from the combination
        ...
        kernel = np.ones(
            (5, 5), np.uint8
        )  # Larger kernel to protect wider edge regions
        ...
        if self.debug:
            print(
                f"Brightness distribution: Bright: {np.sum(bright_mask)/np.sum(non_black)*100:.1f}%, "
        ...
        bright_result = cv2.bilateralFilter(
            image, d_bright, sigma_color_bright, sigma_space_bright
        )
        ...
        dark_result = cv2.bilateralFilter(
            image, d_dark, sigma_color_dark, sigma_space_dark
        )
        ...
        radius = max(
            1, int(2 * (1.0 - preserve_detail_level))
        )  # Small radius to preserve detail
        eps = max(
            0.01, 0.1 * (1.0 - preserve_detail_level)
        )  # Small epsilon for detail preservation
        ...
        cr_denoised = cv2.fastNlMeansDenoising(
            src=cr, dst=None, h=float(h_c), templateWindowSize=7, searchWindowSize=21
        )

        cb_denoised = cv2.fastNlMeansDenoising(
            src=cb, dst=None, h=float(h_c), templateWindowSize=7, searchWindowSize=21
        )
        ...
        non_edge_blend[bright_mask] = cv2.addWeighted(
            bilateral_result[bright_mask],
            alpha_bright,
            nlm_result[bright_mask],
            1.0 - alpha_bright,
            0,
        )
        ...
        non_edge_blend[dark_mask] = cv2.addWeighted(
            bilateral_result[dark_mask],
            alpha_dark,
            nlm_result[dark_mask],
            1.0 - alpha_dark,
            0,
        )
        ...
        result = image.copy()
        result[edge_mask & non_black] = guided_result[
            edge_mask & non_black
        ]  # Edge areas
        result[~edge_mask & non_black] = non_edge_blend[
            ~edge_mask & non_black
        ]  # Non-edge areas
    ...
    def bilateral_filtering(self, image, preserve_detail_level=0.7):
        ...
        sigma_color = max(
            25, int(75 * (1 - preserve_detail_level))
        )  # Color space filter sigma
        sigma_space = max(
            25, int(75 * (1 - preserve_detail_level))
        )  # Coordinate space filter sigma

        if self.debug:
            print(
                f"Applying bilateral filter with parameters: d={d}, sigma_color={sigma_color}, sigma_space={sigma_space}"
            )
    ...
    def denoise_image(
        ...
        if method == "bm3d":
            if self.debug:
                print(
                    f"Using BM3D denoising method {'with' if median_first else 'without'} median filtering"
                )
            ...
            if self.debug:
                print(
                    f"Using median filter only with kernel size {median_kernel_size}x{median_kernel_size}"
                )
            # Create a mask for non-black pixels
            non_black = np.logical_or.reduce(
                (image[:, :, 0] > 5, image[:, :, 1] > 5, image[:, :, 2] > 5)
    ...
    def process_image(
        ...
        try:
            ...
            if self.debug:
                median_info = (
                    f" with median filter ({median_kernel_size}x{median_kernel_size})"
                    if method == "bm3d" and median_first
                    else ""
                )
                print(
                    f"Processed {image_path} -> {output_path} in {processing_time:.2f}s using {method} method{median_info}"
                )
    ...
    def process_directory(
        ...
        print(
            f"Processed {successful} of {total_files} images ({success_rate:.1f}%) using {method} method{median_info}"
        )
        return success_rate
