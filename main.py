...
def process_single_image(image_path, output_dir, denoise_method='bm3d', debug=False):
    ...
    print(f"  Step 2: Removing noise with {noise_methods.get(denoise_method, denoise_method)}...")
    ...
    try:
        ...
        # Save the inpainted image as final result
        ...
        # Save color-corrected image as final result
        result_path = os.path.join(output_dir, filename)
...
def process_directory(input_dir, output_dir, denoise_method='bm3d', debug=False):
    ...
    print(f"Done! Processed {good_count}/{total} images successfully ({success_rate:.1f}%).")

def main():
    # Set up command line options
    parser = argparse.ArgumentParser(
        description="Road Image Enhancement Pipeline"
    )
