...
class PerspectiveCorrector:
    ...
    def find_content_boundaries(self, image):
        ...
        if self.debug:
            ...
            for i, point in enumerate(approx):
                ...
                cv2.putText(debug_img, str(i), tuple(point.reshape(-1)), 
    ...
    def process_image(self, image_path):
        ...
        if self.debug:
            ...
            for i, corner in enumerate(corners):
                cv2.circle(debug_img, (int(corner[0]), int(corner[1])), 5, (0, 255, 0), -1)
                cv2.putText(debug_img, str(i), (int(corner[0]), int(corner[1])), 
                           cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
    ...
    def validate_results(self, input_dir, output_dir):
        ...
        input_images = set(os.path.basename(p) for p in 
                          glob.glob(os.path.join(input_dir, "*.jpg")) + 
                          glob.glob(os.path.join(input_dir, "*.jpeg")) + 
                          glob.glob(os.path.join(input_dir, "*.png")))
        
        output_images = set(os.path.basename(p) for p in 
        ...
        print(f"Validation complete. {len(output_images)}/{len(input_images)} images processed.")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Correct perspective distortion in images.")
    ...
    parser.add_argument("--validate", action="store_true", help="Validate the results after processing.")
