...
parser = argparse.ArgumentParser(
    description='Perform image classification on input images!')
...
# the first 50 images are snow and the next 50 are rain:

for i in range(1, 51):
    snows.append(f'im{str(i).zfill(3)}')
    # Also add the processed image filename pattern
    snows.append(f'processed_im{str(i).zfill(3)}')

for i in range(51, 101):
    rains.append(f'im{str(i).zfill(3)}')
    # Also add the processed image filename pattern
    rains.append(f'processed_im{str(i).zfill(3)}')
...
for filename in names:
    ...
    if img is not None:
        ...
        if(output > 0.5):
            ...
            # Check if this is a snow image (contains -snow in the filename)
            if '-snow' in filename:
                correct += 1
        else:
            print(f'{filename}: rain')
            # Check if this is a rain image (contains -rain in the filename)
            if '-rain' in filename:
                correct += 1
